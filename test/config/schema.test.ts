...
describe("TriageConfig", () => {
  it("accepts a generic config with both views", () => {
    const p = TriageConfig.parse({ source: "github", views: ["code-security", "insights"] });
  ...
  it("accepts a compiled config with a baked scope bag", () => {
    const p = TriageConfig.parse({ source: "github", views: ["code-security"], scope: { repos: ["acme/web"] } });
    expect(p.scope).toEqual({ repos: ["acme/web"] });
  });
  it("rejects an unknown source", () => {
    expect(() => TriageConfig.parse({ source: "gitlab", views: ["code-security"] })).toThrow();
  });
});
