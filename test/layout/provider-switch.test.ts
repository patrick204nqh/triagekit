...
describe("renderProviderSwitch", () => {
  ...
  it("brands by provider (label), not the source id, so a 'github' source still shows the GitHub mark", () => {
    const host = make();
    renderProviderSwitch(host, {
      // The change-request/issue source id is "github" but its provider is "github".
      providers: [{ id: "github", label: "github", on: true, live: true },
                  { id: "gitlab", label: "gitlab", on: false, live: false }],
      onSelect: () => {},
    });
    const chip = host.querySelector<HTMLElement>("[data-prov='github']")!;
