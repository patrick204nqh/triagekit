...
describe("bootstrap composition root", () => {
  ...
  it("wires kinds + store + view and returns a core with refreshNow/rerender", () => {
    ...
    document.body.innerHTML = `<header id="appbar"></header><nav id="domainRail"></nav><nav id="viewswitch"></nav><main id="root"></main><div id="settings-host"></div><div id="delegation-host"></div>`;
    const core = bootstrap(config);
    expect(core).toBeDefined();
    expect(typeof core.refreshNow).toBe("function");
    expect(typeof core.rerender).toBe("function");
    expect(document.querySelector("#delegation-host")).not.toBeNull();
  });
});
